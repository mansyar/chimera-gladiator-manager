...
func _instrument_file(file_entry: Dictionary) -> bool:
	...
	# Use keep_state=true so autoload singletons (active instances) can be
	# reloaded with instrumented code. Godot creates all autoload instances
	# before calling _ready() on any of them, so even though _GDTCoverage is
	# the first autoload, the other autoloads already have instances.
